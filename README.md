...
Every admitted rollout is also stamped with a `scheduler/joint_action_key` that combines the selected scenario/action arm, train cadence, policy-lag limit, active actor cap, and admission-delay bucket. Once a tuple has feedback, `joint_action_objective_weight` adds that tuple's payoff to future rollout scoring for the same runtime context. `ObjectiveScheduler.metrics()` reports rollout, train, stale, score, and objective totals under `scheduler/joint_action/*`, so operators can audit the payoff of the full scheduling action tuple when individual knobs hide interactions.
