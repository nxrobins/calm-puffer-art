...
- `calm_puffer_art.scheduler`: objective-driven rollout/action/train-priority/actor-count/cadence/lag scheduler with action-quality, train-improvement, optional per-arm reward-scale normalization, stale-drop, per-actor attribution, pressure feedback, exploratory runtime-control scoring, and checkpointable control state.
