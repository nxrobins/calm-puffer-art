...
Every admitted rollout is also stamped with a `scheduler/joint_action_key` that combines the selected scenario/action arm, train cadence, policy-lag limit, active actor cap, and admission-delay bucket. `ObjectiveScheduler.metrics()` reports rollout, train, stale, score, and objective totals under `scheduler/joint_action/*`, so operators can audit the payoff of the full scheduling action tuple when individual knobs hide interactions.
