...
class FoundryHarnessTests(unittest.TestCase):
    ...
    def test_public_candidate_manifests_parse(self):
        ...
        frontier_patient_demote = load_foundry_harness_manifest(
        ...
        self.assertEqual(frontier_patient_demote.task_split, "frontier_hard")
        ...
        self.assertEqual(frontier_chunk4.primary_condition, "chunk4_only")
        ...
        self.assertEqual(
            frontier_patient_demote.primary_condition,
            "full_trinity_patient_demote",
        )
        self.assertEqual(
            frontier_patient_demote.conditions,
            ("full_trinity_patient_demote",),
        )
        self.assertEqual(
            frontier_patient_demote.action_codecs,
            ("token", "chunk2", "chunk4"),
        )
        self.assertFalse(frontier_patient_demote.promotion_eligible)
        self.assertEqual(
    ...
    def test_next_hypotheses_recommends_chunk4_probe_after_chunk2_evidence(self):
        comparison = {
            "candidate_aggregates": [
                {
                    "candidate": "frontier_baseline",
        ...
        actions = {action["action"]: action for action in payload["actions"]}
        self.assertIn("run_existing_chunk4_candidate", actions)
        action = actions["run_existing_chunk4_candidate"]
        self.assertEqual(action["candidate"], "frontier_chunk4_only")
        self.assertEqual(
            action["reason"],
            "full_trinity_unstable_lift_after_chunk2_probe",
        )
        self.assertEqual(
    ...
    def test_next_hypotheses_recommends_patient_demote_after_no_demote_evidence(
        self,
    ):
        comparison = {
            "candidate_aggregates": [
                ...
                {
                    "candidate": "frontier_full_trinity_no_demote",
                    "primary_condition": "full_trinity_no_demote",
                    "promotion_eligible": False,
                    "ok_runs": 3,
                    "failure_rate": 0.0,
                    "ranking_score_median": 0.12,
                },
            ],
            "candidate_pairwise": [
                {
        ...
        actions = {action["action"]: action for action in payload["actions"]}
        self.assertIn("reject_existing_no_demote_candidate", actions)
        rejection = actions["reject_existing_no_demote_candidate"]
        self.assertEqual(rejection["candidate"], "frontier_full_trinity_no_demote")
        self.assertEqual(rejection["reason"], "no_demote_probe_underperformed_baseline")
        self.assertIn("run_existing_patient_demote_candidate", actions)
        action = actions["run_existing_patient_demote_candidate"]
        self.assertEqual(
            action["candidate"],
            "frontier_full_trinity_patient_demote",
        )
        self.assertEqual(
            action["reason"],
            "full_trinity_unstable_lift_after_no_demote_probe",
        )
        self.assertEqual(
            action["suggested_lever"],
            "adaptive_action_space_demotion_threshold",
        )
        self.assertIn("frontier_full_trinity_patient_demote", action["command"])
