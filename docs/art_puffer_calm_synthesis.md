...
- The ART bridge stale-checks scheduler-cadenced pending groups before they reach the train ring, so partial ART batches that become too old still fail callers, debit stale feedback, and can run an opt-in demotion-only action-space refresh instead of hiding outside the backpressured queue.
