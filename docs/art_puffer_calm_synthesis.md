...
- It records cadence and policy-lag timing-response decisions under `scheduler/timing_response/*`, keyed by selected value, train-ring pressure bucket, pending-batch bucket, and preference reason, so pressure widening, stale-risk tightening, off-policy tightening, protection, and payoff reuse are auditable separately from the raw numeric cadence/lag value.
...
- It defaults ROI patience to `continuation_objective="accounted"`, so stop/continue decisions divide reward-improving useful experience by rollout, queue, admission, trainer, trainer-wait, and promotion cost accumulated for the train interval; `continuation_objective="train"` remains available for trainer-local compatibility.
