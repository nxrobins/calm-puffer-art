...
- It attributes producer wait on full train-ring admission into scheduler train denominators for both the local runtime and ART bridge, so backpressure reduces marginal improvement per dollar-second instead of only incrementing a queue counter.
