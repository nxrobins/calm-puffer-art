...
- It gates the static actor pool with a learned active actor cap, so actor count becomes a runtime control without changing the user-facing rollout API; actor slots rejected by that cap cancel their unadmitted control probe, and saturation and low-ROI backoff remain initial preferences that actor-count feedback can override after admitted rollout work has scored the cap.
