...
- It can stop training early when `roi_patience` is configured and either train-step objective or accounted interval objective stays below threshold, or when `max_accounted_dollar_seconds` exhausts the configured rollout/train/promotion spend envelope; the ART bridge cancels unspent rollout selections that would exceed the projected accounted budget instead of leaking phantom reservations.
