...
- It stamps each admitted local or ART-bridge rollout with a joint scheduling-action key covering the selected scenario/action arm, cadence, policy-lag limit, actor cap, and admission delay, records the tuple decision at selection time, rolls it back on unspent cancellation, then credits rollout, train, and stale payoff to that tuple under `scheduler/joint_action/*` and reuses matching tuple payoff during future rollout and runtime-control selection.
