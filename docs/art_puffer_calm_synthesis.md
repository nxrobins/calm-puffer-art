...
- It stamps each admitted local or ART-bridge rollout with a joint scheduling-action key covering the selected scenario/action arm, cadence, policy-lag limit, actor cap, and admission delay, then credits rollout, train, and stale payoff to that tuple under `scheduler/joint_action/*`.
