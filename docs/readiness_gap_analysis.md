...
| Semantic bandwidth | Scaffolded. Token, chunk, latent-patch stand-ins, command units, reasoning-step units, and adaptive action-space promotion are implemented. | No learned CALM encoder, trainable chunk logprobs, or reconstruction threshold tied to a real policy loss. | A torch-backed optional encoder that emits old/new/reference logprob evidence and fails closed to token actions. |
...
3. Semantic-bandwidth proof: add an optional learned encoder that satisfies the
   explicit action logprob and reconstruction contract.
