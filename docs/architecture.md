...
- **Coverage floor:** whether under-covered scenario/action arms temporarily override the best current objective estimate as a payoff-bearing coverage-control action, subject to an optional spend-share cap.
...
6. The ring serves the highest-priority non-stale batch to the trainer and records that selected batch as a `scheduler/train_selection/*` decision.
...
- `costs/train_ring_admission_wait_dollar_seconds`: local batcher producer time spent waiting to enter a full train ring. The ART bridge reports the analogous `art_backend/train_ring_admission_wait_dollar_seconds`; both paths include admitted-batch wait in scheduler train cost. When a batch becomes stale before training, its stamped producer wait is charged as scheduler stale-additional dollar-seconds instead of disappearing with the discarded batch.
...
- `costs/accounted_dollar_seconds`: rollout, trainer, trainer-wait, train-ring admission wait, stale-only unobserved sample spend, stale-only train-ring admission overhead, promotion-evaluation, admission-delay, and queue-wait attribution combined.
...
- `north_star/accounted_published_policy_reward_improving_experience_per_dollar_second`: the same published-policy numerator divided by accounted rollout, trainer, trainer-wait, train-ring admission wait, stale-only sample/overhead, promotion-evaluation, admission-delay, and queue-wait spend.
...
- `utilization/*`: rollout, trainer, trainer-wait, train-ring admission wait, queue-wait, and admission-delay seconds divided by wall-clock seconds, exposing serial bottlenecks and parallel actor occupancy.
...
- Deterministic control-dimension sensitivity ablations for policy lag, train-ring capacity, actor count, and token-only action granularity.
