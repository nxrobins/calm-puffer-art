...
Resume uses the same metadata. `restore_control_state()` accepts checkpoint-style metadata, `PolicySnapshot`, or `Checkpoint` objects and loads compatible scheduler/action-space objects. `ControlPlane.run()` calls it automatically when `initial_policy` is a `PolicySnapshot`; the registry seeds from that snapshot's step, checkpoint id, policy object, and metadata. That keeps policy-lag accounting on the resumed version instead of resetting the async runtime to step 0.
...
- `costs/accounted_dollar_seconds`: rollout, trainer, and queue-wait attribution combined.
