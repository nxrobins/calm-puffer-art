...
- `costs/train_ring_admission_wait_dollar_seconds`: local batcher producer time spent waiting to enter a full train ring. The ART bridge reports the analogous `art_backend/train_ring_admission_wait_dollar_seconds`; both paths include admitted-batch wait in scheduler train cost.
...
- `costs/accounted_dollar_seconds`: rollout, trainer, trainer-wait, train-ring admission wait, promotion-evaluation, admission-delay, and queue-wait attribution combined.
