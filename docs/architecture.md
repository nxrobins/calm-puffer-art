...
Cadence and policy-lag selections also record a timing-response key under `scheduler/timing_response/*`. The key includes the control name, selected value, train-ring pressure bucket, pending-batch bucket, and preference reason such as saturation, off-policy tightening, unaccepted-arm protection, positive-signal tightening, or configured default. Local runtime and ART bridge batch tagging preserve those keys on trajectories, so rollout, train, and stale feedback can distinguish "cadence 4 because pressure forced a wider batch" from "cadence 4 because payoff reuse selected it."
